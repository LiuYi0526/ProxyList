...
with open('./last update', 'w') as file:
    current_datetime_update = datetime.now(tz = timezone(timedelta(hours = 8, minutes = 0)))
...
def tg_message_time(div_message):
    ...
    # Change message datetime type into object and convert into China datetime
    datetime_object = datetime.fromisoformat(message_datetime)
    datetime_object = datetime.astimezone(datetime_object, tz = timezone(timedelta(hours = 8, minutes = 0)))

    # Retrieve now datetime based on China timezone
    datetime_now = datetime.now(tz = timezone(timedelta(hours = 8, minutes = 0)))

    # Return datetime object, current datetime based on China datetime and delta datetime
...
url = 'https://raw.githubusercontent.com/LiuYi0526/ProxyList/master/telegram channels.json'
...
# Define update date and time based on China timezone and calendar
datetime_update = datetime.now(tz = timezone(timedelta(hours = 8, minutes = 0)))
datetime_update_str = datetime_update.strftime("LATEST UPDATE: %Y-%m-%d %H:%M:%S %Z").upper()

with open('./proxy_list_pro', 'w') as country_file:
...
for country in country_based_proxies_dict.keys():
    ...
    if country in ["cn", "hk", "sg"]:
        with open('./proxy_list_pro', 'a') as country_file:
            country_file.write("\n".join(country_based_proxies_dict[country]))
            country_file.write("\n")
...
def create_country_table(country_path):
    ...
    country_url_pattern = '[Subscription Link](https://raw.githubusercontent.com/LiuYi0526/ProxyList/master/countries/{country_code}/proxies)'
...
with open('./proxy_list', 'w') as country_file:
    country_file.write(f"{datetime_update_str}\n\n")

with open("./proxy_list", "a", encoding="utf-8") as proxies_file:
